...
const NAV_LINKS = [
  { href: '/banco/solicitudes', label: 'Solicitudes' },
  { href: '/banco/propuestas', label: 'Propuestas' },
  { href: '/banco/proyectos', label: 'Proyectos' },
]
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-white border-b px-6 py-3 flex items-center gap-4 sticky top-0 z-40 shadow-sm">
        <Link href="/banco/solicitudes" className="flex items-center gap-2 mr-4">
          <div className="w-8 h-8 bg-green-700 rounded-full flex items-center justify-center">
            <span className="text-white font-bold text-xs">BN</span>
          </div>
          <span className="font-semibold text-gray-800 hidden sm:block">Banco</span>
        </Link>
          ...
          {NAV_LINKS.map((link) => (
            <Link
              ...
              className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                pathname.startsWith(link.href) ? 'bg-green-50 text-green-700 font-medium' : 'text-gray-600 hover:bg-gray-100'
              }`}
            >
              {link.label}
            </Link>
